...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='Entraînement du système de trading algorithmique')
    
    parser.add_argument('--start-date', type=str, required=True,
                       help='Date de début au format YYYY-MM-DD (ex: 2025-06-02)')
    parser.add_argument('--end-date', type=str, required=True,
                       help='Date de fin au format YYYY-MM-DD (ex: 2025-06-09)')
    parser.add_argument('--symbol', type=str, default='EURUSD',
                       help='Symbole de trading (défaut: EURUSD)')
    parser.add_argument('--timeframe', type=str, default='M5',
                       help='Timeframe (défaut: M5)')
    parser.add_argument('--config', type=str, default='config.yaml',
                       help='Fichier de configuration (défaut: config.yaml)')
    parser.add_argument('--model-type', type=str, default='xgboost',
                       choices=['xgboost', 'lightgbm'],
                       help='Type de modèle ML (défaut: xgboost)')
...
def main():
    ...
    # Noms de fichiers avec type/nom/timeframe/timestamp
    ml_model_filename = f"models/saved/ml_model_{model_type}_{symbol}_{timeframe}_{timestamp}.joblib"
    regime_detector_filename = f"models/saved/regime_detector_{symbol}_{timeframe}_{timestamp}.joblib"
    feature_monitor_filename = f"models/saved/feature_monitor_{symbol}_{timeframe}_{timestamp}.joblib"
    feature_engineer_filename = f"models/saved/feature_engineer_{symbol}_{timeframe}_{timestamp}.joblib"
    ...
    metadata_filename = f"models/saved/metadata_{model_type}_{symbol}_{timeframe}_{timestamp}.json"
    with open(metadata_filename, 'w') as f:
        json.dump(metadata, f, indent=2, default=str)
    
    logger.info(f"Modèles sauvegardés:")
